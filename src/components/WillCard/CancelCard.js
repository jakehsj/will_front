...
function CancelCard(props) {
    ...
    async function cancelWill(id) {
        ...
        try{
            ...
            if(ethereum) {
                ...
                const willtxn = await willControllerContract.cancelWIll(id,{gasLimit:300000});
                await willtxn.wait();
    ...
    async function resetWill(id) {
        ...
        try{
            ...
            if(ethereum) {
                ...
                const signer = provider.getSigner(); //abstraction of ethereum account
                const willControllerContract = new ethers.Contract(contractAddress, contractABI, signer); //abi provides detalis about contract
                const willtxn = await willControllerContract.resetActionBlock(id,{gasLimit:300000});
                await willtxn.wait();
                // will = await willControllerContract.getWill(1);
                // console.log(will);
            } else{
    ...
    return (
        ...
        >
            <Button onClick={async() => cancelWill(props.data.id)}>Cancel</Button>
            <Button onClick={async() => resetWill(props.data.id)}>Reset</Button>
        </Popover>
        </>
    );
    }
